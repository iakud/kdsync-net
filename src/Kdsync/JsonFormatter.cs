...
namespace Kdsync
{
    public static class JsonFormatter
    {
        ...
        private static void WriteTimestamp(TextWriter writer, ref Timestamp timestamp, int indentationLevel)
        {
            WriteBracketOpen(writer, '{');
            bool hasFields = WriteTimestampFields(writer, ref timestamp, indentationLevel + 1);
            WriteBracketClose(writer, '}', true, indentationLevel);
        }

        private static bool WriteTimestampFields(TextWriter writer, ref Timestamp timestamp, int indentationLevel)
        {
            MaybeWriteValueWhitespace(writer, indentationLevel);
            WriteString(writer, "Seconds");
            writer.Write(": ");
            WriteValue(writer, timestamp.Seconds, indentationLevel);
            writer.Write(",");
            MaybeWriteValueWhitespace(writer, indentationLevel);
            WriteString(writer, "Nanos");
            writer.Write(": ");
            WriteValue(writer, timestamp.Nanos, indentationLevel);
            return true;
        }

        private static void WriteDuration(TextWriter writer, ref Duration duration, int indentationLevel)
        {
            WriteBracketOpen(writer, '{');
            bool hasFields = WriteDurationFields(writer, ref duration, indentationLevel + 1);
            WriteBracketClose(writer, '}', true, indentationLevel);
        }

        private static bool WriteDurationFields(TextWriter writer, ref Duration duration, int indentationLevel)
        {
            MaybeWriteValueWhitespace(writer, indentationLevel);
            WriteString(writer, "Seconds");
            writer.Write(": ");
            WriteValue(writer, duration.Seconds, indentationLevel);
            writer.Write(",");
            MaybeWriteValueWhitespace(writer, indentationLevel);
            WriteString(writer, "Nanos");
            writer.Write(": ");
            WriteValue(writer, duration.Nanos, indentationLevel);
            return true;
        }

        private static void WriteEmpty(TextWriter writer, ref Empty empty, int indentationLevel)
        {
            writer.Write("{}");
        }
        ...
        public static void WriteValue(TextWriter writer, object value, int indentationLevel)
        {
            ...
            {
                WriteTimestamp(writer, ref timestamp, indentationLevel);
            }
            else if (value is Duration duration)
            {
                WriteDuration(writer, ref duration, indentationLevel);
            }
            else if (value is Empty empty)
            {
                WriteEmpty(writer, ref empty, indentationLevel);
            }
            else
            {
